...
class GraphVis(QWidget):
    ...
    def __init__(self, graph: Graph):
        ...
        # pprint(self.graph.connections)
        for node, connections in self.graph.connections.items():
            for idx, connection in enumerate(connections):
                if connection is None:
                    continue
                connection_node, connection_idx = connection
                self.add_connection(node, idx, connection_node, connection_idx)
    ...
    def add_node(self, node: Node, add_to_graph: bool = True, x: float = 0, y: float = 0):
    ...
    def delete_node(self):
        ...
        node = sender.node
        # delete connections:
        for i in range(len(node.parameter_template)):
            self.parameter_connections[(node, i)].delete.emit()
        for i in range(len(node.result_template)):
            self.result_connections[(node, i)].delete.emit()
    ...
    def handle_mouse_press(self, event: QGraphicsSceneMouseEvent):
        ...
        if not items_under_mouse or (len(items_under_mouse) == 1 and
                                     isinstance(items_under_mouse[0], ConnectionVis)):
            self.remove_temp_connection()
    ...
    def add_connection(self, parameter_node: Node, parameter_idx: int, result_node: Node, result_idx: int):
        print(parameter_node, parameter_idx, result_node, result_idx)
